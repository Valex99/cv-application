import { useState } from "react";
import { BsX } from "react-icons/bs";
import TextInput from "../form-components/TextInput";
import TextArea from "../form-components/TextAreaInput";
import SaveButton from "../form-components/SaveButton";
import { v4 as uuidv4 } from "uuid"; // Import UUID

export default function EditExistingExperience({
  selectedToEdit,
  setSelectedToEdit,
  onClose,
  allExperienceArray,
  setAllExperienceArray,
}) {
  const [temporary, setTemporary] = useState(selectedToEdit);

  const handleSubmit = (e) => {
    e.preventDefault();

    // Find project with that id inside allExperienceArray
    // And replace it with temporary

    setSelectedToEdit(null);
    onClose();
  };

  return (
    <div
      className="fixed inset-0 flex justify-center items-center z-50 bg-gray-700/40 backdrop-blur-sm z-10"
      onClick={onClose}
    >
      <div
        className="bg-white p-8 rounded-lg w-[400px]"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-2xl font-bold">Work</h2>
          {/* Close Button (X) */}
          <div
            onClick={onClose}
            className="w-10 h-10 rounded-full flex justify-center items-center bg-white hover:bg-gray-300 active:bg-gray-400 text-white cursor-pointer"
          >
            <BsX className="text-black h-8 w-8" />
          </div>
        </div>
        <form onSubmit={handleSubmit}>
          <TextInput
            id="title"
            label="Job Title"
            value={temporary.title}
            onChange={(e) =>
              setTemporary((prev) => ({
                ...prev,
                title: e.target.value,
              }))
            }
          />

          <TextInput
            id="employeer"
            label="Employeer"
            value={temporary.employeer}
            onChange={(e) =>
              setTemporary((prev) => ({
                ...prev,
                employeer: e.target.value,
              }))
            }
          />

          <TextArea
            id="responsibilities"
            label="Description"
            value={temporary.responsibilities}
            height="100"
            onChange={(e) =>
              setTemporary((prev) => ({
                ...prev,
                responsibilities: e.target.value,
              }))
            }
          />

          {/* Start Date */}
          <div className="mb-4">
            <label className="block mb-1">Start Date</label>
            <div className="flex gap-4">
              {/* Month Selector */}
              <select
                className="w-full p-2 border border-gray-300 rounded"
                value={temporary.startDate?.month || ""}
                onChange={(e) =>
                  // To update Nested Properties (object inside object)
                  setTemporary((prev) => ({
                    ...prev,
                    startDate: { ...prev.startDate, month: e.target.value },
                  }))
                }
              >
                <option value="">Select Month</option>
                {[
                  "January",
                  "February",
                  "March",
                  "April",
                  "May",
                  "June",
                  "July",
                  "August",
                  "September",
                  "October",
                  "November",
                  "December",
                ].map((month) => (
                  <option key={month} value={month}>
                    {month}
                  </option>
                ))}
              </select>

              {/* Year Selector */}
              <input
                type="number"
                className="w-full p-2 border border-gray-300 rounded"
                min="1950"
                max="2100"
                step="1"
                value={temporary.startDate?.year || ""}
                onChange={(e) =>
                  setTemporary((prev) => ({
                    ...prev,
                    startDate: { ...prev.startDate, year: e.target.value },
                  }))
                }
              />
            </div>
          </div>

          <div className="mb-4">
            <label className="block mb-1">End Date</label>
            <div className="flex gap-4">
              {/* Month Selector */}
              <select
                className="w-full p-2 border border-gray-300 rounded"
                value={temporary.endDate?.month || ""}
                onChange={(e) =>
                  setTemporary((prev) => ({
                    ...prev,
                    endDate: { ...prev.endDate, month: e.target.value },
                  }))
                }
              >
                <option value="">Select Month</option>
                {[
                  "January",
                  "February",
                  "March",
                  "April",
                  "May",
                  "June",
                  "July",
                  "August",
                  "September",
                  "October",
                  "November",
                  "December",
                ].map((month) => (
                  <option key={month} value={month}>
                    {month}
                  </option>
                ))}
              </select>

              {/* Year Selector */}
              <input
                type="number"
                className="w-full p-2 border border-gray-300 rounded"
                min="1950"
                max="2100"
                step="1"
                value={temporary.endDate?.year || ""}
                onChange={(e) =>
                  setTemporary((prev) => ({
                    ...prev,
                    endDate: { ...prev.endDate, year: e.target.value },
                  }))
                }
              />
            </div>
          </div>

          {/* Buttons */}
          <SaveButton />
        </form>
      </div>
    </div>
  );
}
